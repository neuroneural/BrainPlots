...
project_folders = sorted([
    'csrf','pialnn', 'topofit', 'cortexode_rk4', 'deepcsr',
...
fig, axes = plt.subplots(1, 2, figsize=(2 * subplot_aspect * subplot_height, subplot_height))
...
for i, surface_type in enumerate(['pial', 'white']):
    ax = axes[i]
    data = all_data[all_data['Surface Type'] == surface_type]
    ...
    ax.set_title(f'{surface_type.capitalize()} Surface - Self Intersections')
    ax.set_xscale('log')
...
plt.tight_layout()
plt.savefig('combined_self_intersections_scatter.png')
plt.savefig('combined_self_intersections_scatter.svg')
plt.close()
