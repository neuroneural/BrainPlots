...
def create_stl_file_path(subject_id, hemisphere, surface_type):
    return f"cortexode_{subject_id}_C_mwrm_{hemisphere}_{surface_type}.stl"

# Function to process each .ply file
def process_ply_file(file):
    subject_id, hemisphere, surface_type = re.findall(r'cortexode_(\d+)_mw_(lh|rh)_(pial|white)', file)[0]
...
ply_files = [file for file in glob.glob(f"{directory_path}/cortexode_*_mw_*_*.ply")]
